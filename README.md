...
- En un pedido `APPROVED`, `Generar archivos` crea `PEDIDO-1.pdf` en Planteamientos y un `.xls` de reserva por cada OF en Subida de material. En la carpeta anual TOLDOS sustituye el borrador por el mismo planteamiento definitivo con el nombre `PEDIDO.pdf`; el documento conserva adjuntos los datos necesarios para reabrirlo desde la web. Si algún archivo existe, pide confirmación antes de sustituirlo y después conserva en la ficha la fecha, el autor y las rutas generadas.
