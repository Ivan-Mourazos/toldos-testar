    ...
    {editing && <>
      ...
      <div className="structure-edit-scroll"><table className="despiece-table structure-edit-table"><thead><tr><th>Nº</th><th>Pieza / artículo RPS</th><th>Unidades</th><th>Corte (cm)</th><th>Reserva RPS</th>{editor.stale && <th>Cálculo actual</th>}<th>Acciones</th></tr></thead>
        <tbody>{rows.map((row) => { const original = editor.baseRows.find((item) => item.id === row.id); const changed = !original || ['name', 'reference', 'units', 'length', 'reservationQuantity'].some((key) => row[key as keyof StructureRow] !== original[key as keyof StructureRow]); return <tr key={row.id} className={changed ? 'structure-row-modified' : ''}>
          ...
          <td><input aria-label={'Descripción fila ' + row.num} value={row.name} onChange={(event) => updateRow(row.id, { name: event.target.value })} /><small>{row.reference || 'Sin referencia de reserva'}</small></td>
          ...
          <td><div className="structure-row-actions"><button type="button" aria-label={'Buscar o sustituir artículo RPS de la fila ' + row.num} title="Buscar o sustituir en RPS" onClick={() => setSearchRow(row.id)}><Search aria-hidden="true" /></button><button type="button" aria-label={'Eliminar fila ' + row.num} title="Eliminar pieza" onClick={() => setRows((current) => current.filter((item) => item.id !== row.id))}><Trash2 aria-hidden="true" /></button></div></td>
        </tr>; })}</tbody></table></div>
